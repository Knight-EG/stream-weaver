...
import { Tv, Search, X, ArrowLeft, Grid3X3, List, SlidersHorizontal } from 'lucide-react';
...
export default function Series() {
  ...
  const [activeEpisode, setActiveEpisode] = useState<Channel | null>(null);
  ...
  const series = useMemo(() => {
    return playlist.channels.filter(ch => ch.type === 'series');
  }, [playlist.channels]);

  const seriesCategories = useMemo(() => {
    const cats = new Set<string>();
    series.forEach(s => { if (s.group) cats.add(s.group); });
    return Array.from(cats).sort();
  }, [series]);

  const filtered = useMemo(() => {
    let list = series;
    if (selectedCategory) list = list.filter(s => s.group === selectedCategory);
    if (searchQuery) {
    ...
    return list;
  }, [series, selectedCategory, searchQuery, sortBy]);
  ...
  if (activeEpisode) {
    return (
      <div className="min-h-screen bg-background">
        <VideoPlayer url={activeEpisode.url} title={activeEpisode.name} channelId={activeEpisode.tvgId || activeEpisode.id} onBack={() => setActiveEpisode(null)} />
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      <header className="border-b border-border bg-card px-4 py-3 flex items-center gap-3">
        ...
        <h1 className="text-lg font-bold text-foreground">{t('seriesTitle')}</h1>
      ...
      <div className="px-4 py-3 border-b border-border bg-card/50 space-y-3">
        <div className="flex gap-2">
          <div className="relative flex-1">
            ...
            <input type="text" placeholder={t('seriesSearchPlaceholder')} value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
      ...
      <div className="flex-1 p-4 overflow-y-auto">
        {filtered.length === 0 ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3">
            {filtered.map(ep => (
              <button key={ep.id} onClick={() => setActiveEpisode(ep)} className="group bg-card border border-border rounded-xl overflow-hidden hover:border-primary/50 hover:shadow-lg transition-all tv-focusable" data-focusable="true">
          ...
          <div className="space-y-1">
            {filtered.map(ep => (
              <button key={ep.id} onClick={() => setActiveEpisode(ep)} className="w-full flex items-center gap-3 p-3 rounded-lg bg-card border border-border hover:border-primary/50 transition-all tv-focusable" data-focusable="true">
